...
import { TrendingUp, Home, AlertCircle, FileText, MapPin, ArrowRight, Zap, Calendar } from 'lucide-react';
...
const Dashboard = () => {
    ...
    // Pendientes que requieren atención — Serial Position: mora primero
    const requierenAtencion = pagos
        .filter(p => p.estado === 1 || p.estado === 3 || p.estado === 4)
        .sort((a, b) => b.estado - a.estado) // mora(3) > parcial(4→reordered) > pendiente(1)
        .sort((a, b) => {
            const order = { 3: 0, 4: 1, 1: 2 };
            return (order[a.estado] ?? 3) - (order[b.estado] ?? 3);
        })
        .slice(0, 5); // Miller's Law: máximo 5

    const kpis = [
        { label: 'Ingresos Totales',     value: '$' + parseFloat(resumen?.ingresos_totales || 0).toLocaleString(), icon: <TrendingUp size={20} color="#22c55e" />, bg: '#dcfce7', trend: '+' },
    ...
    return (
        <div className="fade-in">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.4rem' }}>
                    <button onClick={ejecutarMotor} disabled={ejecutando} className="btn btn-primary"
                        ...
                        <Zap size={16} />
                        {ejecutando ? 'Ejecutando...' : 'Motor Financiero'}
                    </button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem', marginBottom: '1.75rem' }}>
                {kpis.map((k, i) => (
                    <div key={i} className="card" style={{ margin: 0, position: 'relative', overflow: 'hidden',
                        ...(k.alert ? { borderColor: '#fca5a5', boxShadow: '0 0 0 2px rgba(239,68,68,0.1)' } : {}) }}>
            ...
            {/* Sección Requieren Atención — Von Restorff + Fitts + Serial Position */}
            {requierenAtencion.length > 0 && (
                <div className="card" style={{ margin: 0, padding: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', background: '#fafafa' }}>
                        ...
                        {/* Peak-End Rule: CTA clara al final del header */}
                        <button onClick={() => navigate('/pagos')} style={{
                            display: 'flex', alignItems: 'center', gap: '0.4rem',
                            background: 'none', border: '1.5px solid #e2e8f0', borderRadius: '0.5rem',
                            padding: '0.4rem 0.875rem', cursor: 'pointer', fontSize: '0.82rem',
                            fontWeight: '600', color: '#475569', transition: 'all 0.15s'
                        }}
                    ...
                    {requierenAtencion.map((pago, idx) => {
                        ...
                        return (
                            <div key={pago.id_pago} style={{
                                ...
                                background: cfg.bg,
                                borderBottom: idx < requierenAtencion.length - 1 ? `1px solid ${cfg.border}` : 'none',
                                transition: 'background 0.15s',
                            }}
                            onMouseEnter={e => e.currentTarget.style.filter = 'brightness(0.97)'}
                            onMouseLeave={e => e.currentTarget.style.filter = 'none'}>

                                {/* Indicador de urgencia — Von Restorff */}
                                <div style={{ width: '4px', height: '40px', borderRadius: '2px', background: cfg.dot, flexShrink: 0 }} />

                                {/* Info del inmueble — Proximidad */}
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                                        ...
                                        <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                                            Arrendatario: {pago.Contrato?.id_inquilino || '--'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                    <div style={{ fontWeight: '800', fontSize: '1rem', color: esMora ? '#b91c1c' : '#0f172a' }}>
                                        ${parseFloat(pago.saldo_pendiente).toLocaleString()}
                                    </div>
                                    <span style={{ fontSize: '0.7rem', background: cfg.badge, color: cfg.badgeText, padding: '0.1rem 0.45rem', borderRadius: '999px', fontWeight: '700' }}>
                                        {cfg.label}
                                    </span>
                                </div>

                                {/* CTA — Fitts: botón grande y accesible */}
                                <button
                    ...
                    {/* Si hay más de 5 */}
                    {pagos.filter(p => p.estado === 1 || p.estado === 3 || p.estado === 4).length > 5 && (
                        <div style={{ padding: '0.875rem 1.5rem', textAlign: 'center', background: '#f8fafc', borderTop: '1px solid #f1f5f9' }}>
                            <button onClick={() => navigate('/pagos')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#2563eb', fontWeight: '600', fontSize: '0.85rem', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
                                Ver todos los cobros pendientes <ArrowRight size={14} />
                            </button>
                        </div>
                    )}
                </div>
            ...
            {/* Estado limpio — Peak-End: si todo está al día, terminar con mensaje positivo */}
            {requierenAtencion.length === 0 && pagos.length > 0 && (
                <div style={{ textAlign: 'center', padding: '2rem', background: '#f0fdf4', borderRadius: '1rem', border: '1px solid #bbf7d0' }}>
                    ...
                    <p style={{ fontWeight: '700', color: '#15803d', fontSize: '1rem' }}>¡Todo al día!</p>
                    <p style={{ color: '#4ade80', fontSize: '0.875rem' }}>No hay cobros pendientes en este momento.</p>
                </div>
            )}
        </div>
    );
